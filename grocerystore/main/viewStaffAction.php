    ...
    <div class="container-fluid">
      <div class="row">
        <div class="col-sm-3 col-md-2 sidebar">
          ...
          <ul class="nav nav-sidebar">
            <li class="active"><a href="viewStaff.html">View Staff Info<span class="sr-only">(current)</span></a></li>
  ...
  echo "<a href=\"javascript:history.go(-1)\">Return to Previous Page</a>";
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			echo 'Position: ' . $row['position'] . ' - Name: ' . $row['first_name'] . ' ' . $row['last_name'] . ' - Phone: ' . $row['phone_number'] . ' - Email: ' . $row['email_address'] . ' - Hire Date: ' . $row['hire_date'] . ' - Wage: ' . $row['wage'] . '<br>';
		}
	} else {
		echo "0 results <br>";
	}
