      ...
      <div class="row">
        <div class="col-sm-3 col-md-2 sidebar">
          <ul class="nav nav-sidebar">
            ...
            <li class="active"><a href="viewSKU.html">View SKU Information<span class="sr-only">(current)</span></a></li>
  ...
  echo "<a href=\"javascript:history.go(-1)\">Return to Previous Page</a>";
	...
	if ($result->num_rows > 0) {
		while($row = $result->fetch_assoc()) {
			echo 'SKU: ' . $row['SKU']. ' - Item Dept.: ' . $row['item_dept']. ' - Item Name:  ' . $row['item_name']. ' - Avg Monthly Sales: ' . $row['average_monthly_sale']. ' - Reorder Quantity: ' . $row['reorder_quantity']. ' - Retail Price: ' . $row['retail_sell_price']. ' - Discount Price: ' . $row['discount_price']. ' - Monthly Sales: ' . $row['sales']. '<br>';
		echo "<a href=\"javascript:history.go(-1)\">Return to Previous Page</a>";
		}
	} else {
		echo '0 results';
